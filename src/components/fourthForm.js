...
import ValidateV2 from "../helpers/validateV2"
import validateV2 from "../helpers/validateV2";
...
class FourthForm extends Component {
    ...
    constructor(props) {
        ...
        validateV2.prototype.addMethod("passwordRegex", function (value) {
            var regex = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,}$/
            return regex.test(value);
        }, "parola kurallarına uymuyor");

        validateV2.prototype.addMethod("passwordRegex2", function (value) {
            var regex = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,}$/
            return regex.test(value);
        }, "parola kurallarına uymuyor2");
    ...
    handleSubmit(event) {
        ...
        const {formIsValid, validateObject} = new ValidateV2(this.state.validate, this.state.form).valid();

        console.log("formIsValid", formIsValid,"validateObject",validateObject,);
    ...
    componentDidMount() {
      ...
      //let validate1=  new ValidateV2(this.state.validate,this.state.form);

       /* validate1.addMethod("passwordRegex", function (value) {
            var regex = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,}$/
            return regex.test(value);
        }, "parola kurallarına uymuyor");
        validate1.addWithAttr(this.state.validate, this.state.form);*/


       // window.validate1=validate1;
    }
